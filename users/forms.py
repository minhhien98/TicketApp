...
class RegisterForm(forms.Form):
    username = forms.CharField(label="Tên Tài khoản:", required=True,min_length=5, max_length=20, widget=forms.TextInput(attrs={ 'class':"form-control form-control-lg", 
                                    'placeholder':"Tên tài khoản"}))
    password = forms.CharField(label="Mật khẩu:", required=True, min_length=5, widget=forms.TextInput(attrs={ 'type':'password', 'class':"form-control form-control-lg", 
                                    'placeholder':"Mật khẩu"}))
    confirm_password = forms.CharField(label="Xác nhận mật khẩu:",required=True, widget=forms.TextInput(attrs={ 'type':'password','class':"form-control form-control-lg", 
...
class ChangePasswordForm(forms.Form):
    old_password = forms.CharField(label ='Mật khẩu cũ:',required=True, min_length=5, widget=forms.TextInput(attrs={ 'type':'password', 'class':"form-control form-control-lg", 
    ...
    def clean(self):
        ...
        if old_password == new_password:
            raise forms.ValidationError('Xin vui lòng không dùng mật khẩu cũ!')
        if new_password != confirm_password:
            raise forms.ValidationError('Mật khẩu xác nhận không đúng!')
        return data

class VerifyEmailForm(forms.Form):
    email = forms.EmailField(label = 'Email:',required=True,error_messages={'required':'Xin vui lòng điền thông tin.','invalid':'Xin vui lòng nhập đúng email của bạn.'})
    confirm_email = forms.EmailField(label='Xác nhận Email:',required=True,error_messages={'required':'Xin vui lòng điền thông tin.','invalid':'Xin vui lòng nhập đúng email của bạn.'})
    ...
    def clean(self):
        ...
        if str(email) != str(confirm_email):
            raise forms.ValidationError('Email không trùng khớp.')
        return data